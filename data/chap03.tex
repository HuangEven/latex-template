...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig4_1_single_gpu_system_architecture.pdf}
...
尽管第三章已经完成了 FlashTOD 执行流程的构建，但从系统执行效率看，原始链路仍存在显著瓶颈。问题在于这些阶段的衔接方式仍然保留了典型的``CPU 主导分段执行''特征：查询特征多在 CPU 侧整理，候选结果在 GPU 上产生后又返回 CPU 重组，后续异常评分再重新依赖 GPU。这样一来，原本分散在不同阶段边界上的开销会在 FlashTOD 链路中叠加出现：传输代价增加，同步点变多，等待时间也被层层放大。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig4_2_original_vs_gpu_dominant_chain.pdf}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig4_3_flashanns_async_io_compute_pipeline.pdf}
...
后端优化围绕热路径展开：字段筛选、类型转换和窗口统计尽量前移到 RAPIDS/cuDF；候选组织、批量拼接和统计聚合转为 GPU DataFrame 或 CuPy 张量表达；临时内存与必要的主机交换缓冲由 RMM 内存池和 pinned host 资源管理；与 PyTOD、检索后端交接时，则优先采用设备侧连续缓冲，避免``DataFrame---Python 对象---Tensor''的反复转换路径\citep{rapids_rmm_pinned}。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{plot_assets/ch04_single_gpu_overview/fig4_16_single_gpu_overview.pdf}
  \caption{单卡性能综合对比}
...
本章围绕单 GPU 场景下的系统实现问题，分析了原始 FlashTOD 链路中存在的边界交换、候选重组、I/O 等待以及资源管理抖动等瓶颈，并在此基础上提出了 GPU 主导的端到端执行链路。围绕这条链路，本文进一步引入 RAPIDS 与 FAISS-GPU 对数据处理与候选检索后端进行工程增强，并通过异步 stream、双缓冲、内存池与统一监控机制补充其稳定性与可观察性。
