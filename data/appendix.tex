...
FlashANNS & 完整 FlashTOD 路径中的主要 ANNS 后端，在本文中承担候选召回前端，用于大规模外存驻留或超显存索引条件下的高吞吐候选生成。 \\
初步融合方案 & FAISS-GPU ANN API 与 PyTOD 的简单串接，用于验证候选召回---候选评分链路可行性。 \\
完整融合方案 & 以 FlashANNS 为主要 ANNS 后端、以 PyTOD 为异常评分主干，并结合 GPU 主导数据路径和设备侧候选组织后的完整 FlashTOD 实现。 \\
