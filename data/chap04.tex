...
\begin{figure}[!htbp]
  ...
  \caption{数据并行与索引复制多 GPU 架构（本文绘制）}
...
\begin{figure}[!htbp]
  ...
  \caption{CPU 控制面调度与微批示意（本文绘制）}
  \label{fig:control-plane}
\end{figure}

图中 FAISS-GPU 表示前期候选召回接口验证路径；多 GPU 完整路径下的本地候选生成以后续接入的 FlashANNS 检索后端为主。
...
\begin{figure}[!htbp]
  ...
  \caption{多 GPU 扩展趋势与共享路径瓶颈分析（实验数据绘制）}
...
\begin{figure}[!htbp]
  ...
  \caption{I/O 通道配置对扩展性的影响（实验数据绘制）}
