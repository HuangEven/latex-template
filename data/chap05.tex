...
因此，本章将数据口径分为两类：真实集和 1M 桥接实验沿用固定随机种子的训练/验证/测试划分，用于比较 PR-AUC、Recall@$k$ 与 Top-$k$ overlap；10M、50M、100M 性能实验则固定索引集与查询集，持续注入请求以观察 QPS、p99 延迟和资源利用率，从而避免把检测效果评估与系统压测混在同一口径下。
...
为使差异尽量来自系统设计而非实验噪声，本文在同类实验中固定数据源、标签口径、有效性划分、索引集和查询集；不同方案共享候选预算、批大小、硬件平台和 CUDA、PyTorch、FAISS-GPU、RAPIDS、NCCL 等软件栈。单卡与多 GPU 只在相同规模下分别比较，并按统一重复轮次和统计方式汇总。
...
因此，本文结论应理解为单机多 GPU 条件下的工程验证：replicated 主方案仍受显存与索引规模约束，out-of-core 和拓扑感知 I/O 的收益依赖具体硬件路径与 GDS 支持，超大规模真实金融负载验证仍有限。多机分布式场景中的跨机通信、分布式索引和集群调度尚未纳入本文范围，需要在后续工作中继续验证。
