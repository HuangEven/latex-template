...
这一组结果回答“FlashTOD 本身是否值得成立”。比较对象包括 PyTOD 全量评分、仅召回方案、初步融合方案以及完整融合方案。若完整融合在 PR-AUC 或 Recall@$k$ 上与全量评分保持接近，却在 QPS 上明显领先，就可以说明“候选召回压缩评分范围 + 候选集上张量化评分”这一结构在系统层面是有意义的。
...
为便于综合展示，表~\ref{tab:core-summary} 汇总了核心结果，图~\ref{fig:fusion-vs-baselines}、图~\ref{fig:single-gpu-before-after}、图~\ref{fig:multi-gpu-scaling} 分别从检测效果、吞吐能力和效果--性能折中三个视角对 FlashTOD 与基线方案进行比较，图~\ref{fig:scale-comparison} 汇总单卡优化与多 GPU 扩展的收益差异，而图~\ref{fig:scale-trend} 则展示跨规模趋势。
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.8\linewidth]{plot_assets/ch06_fusion_compare/fig6_4_fusion_accuracy.pdf}
  \caption{FlashTOD 与基线系统检测效果对比}
...
本章的综合实验把全文前几章分散建立的判断收束到同一验证框架中。结果表明，FlashTOD 在保持合理检测效果的同时，确实形成了更适合大规模金融异常检测的系统组织方式；单卡优化的主要收益来自候选召回到异常评分之间的数据路径压缩；多 GPU 条件下，replicated 更适合作为吞吐优先场景的主方案，而 GPU merge 与拓扑感知 I/O 控制则主要用于容量受限或路径受限场景下维持扩展效果。
