...
\textbf{统计稳定性与重复运行限制。}当前实验结果主要来自固定硬件、固定查询集、固定候选预算和固定批处理参数下的稳定运行记录。受实验周期限制，正文尚未对每一种规模、每一类归并路径和每一个候选预算组合提供完整重复轮次、均值、标准差、误差棒或显著性检验。因此，本文对实验结果的解释聚焦于趋势性差异和工程可行性，不将当前图表中的数值差异直接解释为统计显著结论。

\textbf{外部基线覆盖范围限制。}本文基线主要围绕 FlashTOD 内部路径展开，用于比较全量评分、仅召回、FAISS-GPU 初步串接、FlashANNS 完整链路以及不同多 GPU 归并策略之间的差异。尚未在相同数据、相同候选预算和相同评分主干下系统纳入 HNSW、IVF/IVF-PQ、DiskANN、PyOD CPU 实现等外部方案，因此本文不声称对全部 ANNS 前端或异常检测系统形成全面优势。后续工作可按表~\ref{tab:external-baseline-plan} 所示口径补充外部基线。
