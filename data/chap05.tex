...
软件环境方面，实验系统由 PyTOD 评分实现、FlashANNS 候选召回模块、RAPIDS 数据处理组件、FAISS-GPU 检索后端增强组件以及多 GPU 通信和调度运行时共同组成。为了保证结果可复核，本文固定 Python、PyTorch、PyTOD、FAISS-GPU、CUDA、NCCL 以及操作系统版本，并把这些版本视为统一的软件环境快照。
...
本章采用的指标体系服务于全文的四条核心判断：FlashTOD 是否在效果上可接受，单卡优化是否主要压缩了数据路径，多 GPU replicated 是否真正提升了吞吐，以及 I/O 路径是否决定扩展上限。围绕这四类判断，指标被组织为四组：异常检测效果指标、候选检索质量指标、系统吞吐与延迟指标以及资源利用率指标。
...
为了回应不同层次的实验问题，本文将对比方案组织为从效果上界、模块基线到完整系统方案的递进关系，如表~\ref{tab:baseline-settings} 所示。PyTOD 全量评分主要用于提供小规模条件下的检测效果上界和全量评分基线；仅召回方案用于判断候选召回本身是否足以完成异常检测；初步融合方案用于刻画 FlashANNS 与 PyTOD 简单串接时的系统表现；完整融合方案 FlashTOD 则对应本文经过数据路径组织和后端优化后的完整实现。多 GPU 场景下，CPU aggregation 与 GPU merge 分别作为主机侧归并和设备侧归并的对照，用于分析跨卡结果组织带来的额外代价。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{P{2.4cm}P{2.3cm}P{2.3cm}P{3.6cm}P{4.5cm}}
    ...
    仅召回方案 & 使用 FlashANNS/ANN 候选召回 & 不进入完整评分主干 & 只观察候选集合与排序结果 & 验证仅依靠召回是否不足以替代异常评分\\
    ...
    CPU aggregation & 多 GPU 本地召回 & 多 GPU 本地评分 & 跨卡结果回到 CPU 侧排序与聚合 & 作为主机侧归并路径的对照\\
    GPU merge & 多 GPU 本地召回 & 多 GPU 本地评分 & 结果尽量留在 GPU 侧并借助通信库归并 & 评估设备侧归并相对 CPU 聚合的收益和代价\\
    \bottomrule
  \end{tabular}
  \end{adjustbox}
\end{table}

在统计口径上，当前实验结果基于固定硬件平台、固定索引集、固定查询集、固定候选预算和固定批大小下的稳定运行结果。由于送审前实验时间有限，本文尚未对所有配置给出完整误差棒和显著性检验，因此正文图表主要展示均值或稳定运行趋势，结论也主要强调不同方案之间的趋势性差异。后续若进一步完善实验，应在附录或实验日志中补充各配置的重复运行记录、标准差范围和必要的统计检验。
...
比较时，同类实验固定数据源、标签口径、有效性划分、索引集和查询集；不同方案共享候选预算、批大小、硬件平台以及 CUDA、PyTorch、FAISS-GPU、RAPIDS、NCCL 等软件栈。单卡与多 GPU 仅在相同规模下比较，并统一重复轮次和统计方式。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{P{2.4cm}P{2.1cm}P{2.6cm}P{4.8cm}P{2.0cm}}
    ...
    单卡链路优化收益 & 单卡优化 & QPS, p99 Latency, CPU Utilization & p99 延迟和 CPU 负载下降，端到端吞吐提升 & 图~\ref{fig:single-stage-breakdown}--图~\ref{fig:single-gpu-overview}，图~\ref{fig:scale-comparison} \\
    多 GPU replicated 扩展性 & 2/4 GPU replicated & QPS, CPU Utilization & QPS 随卡数提升，CPU 利用率未同步放大 & 图~\ref{fig:qps-vs-gpu}，图~\ref{fig:scale-comparison} \\
    跨卡归并策略差异 & GPU merge vs.\ CPU aggregation & avg/p99 Latency & GPU 侧归并的平均延迟和 p99 延迟更低 & 图~\ref{fig:latency-quantiles}，表~\ref{tab:primary-vs-fallback} \\
...
图~\ref{fig:fusion-vs-baselines} 用于同时观察不同系统配置在检测效果和吞吐率上的差异。PyTOD 全量评分提供小规模条件下的效果参考，但其全量评分成本较高，并不适合作为大规模吞吐实验的统一对照；仅召回方案虽然吞吐较高，但 PR-AUC 明显低于带有异常评分的方案，说明候选召回本身不能替代评分主干；初步融合方案介于仅召回和完整方案之间，而完整融合方案 FlashTOD 在 PR-AUC 与 QPS 之间取得更均衡的表现，体现了候选召回、张量化评分和数据路径组织共同作用后的系统收益。
...
从综合结果看，本文形成的四条主结论之间并不是彼此独立的。FlashTOD 之所以成立，并不仅仅因为它“比基线快”，而在于它在效果和吞吐之间建立了更稳定的平衡关系：候选召回没有明显破坏异常评分主干的判别能力，同时又把后续评分范围控制在系统可承受的预算内。对金融异常检测系统来说，这一点比单纯追求某个离线指标更重要，因为它决定了系统有机会进入真实高并发场景。
...
\]
其中，$T_{\mathrm{prep}}$ 表示查询准备时间，$T_{\mathrm{retrieve}}$ 表示候选召回时间，$T_{\mathrm{pack}}$ 表示候选组织与张量化时间，$T_{\mathrm{score}}$ 表示 PyTOD 评分时间，$T_{\mathrm{merge}}$ 表示多 GPU 归并时间，$T_{\mathrm{io}}$ 表示外存访问与主机---设备传输开销。按这一分解看，单卡优化主要降低 $T_{\mathrm{pack}}$ 和 $T_{\mathrm{io}}$，异步 I/O 流水线主要压缩 $T_{\mathrm{retrieve}}$ 中的等待成分；多 GPU replicated 则通过查询级并行降低单位查询分摊的检索和评分成本，而 GPU merge 或 CPU aggregation 会额外引入 $T_{\mathrm{merge}}$。

单卡优化的意义也不只是数值上的提速。第四章和本章结果共同表明，系统收益主要来自数据路径被压缩之后，候选召回与异常评分之间的边界成本下降，CPU$\leftrightarrow$GPU 传输次数与阶段同步等待被压到更低水平。这意味着，原先限制系统的并不是某一个算子绝对不够快，而是模块之间的折返路径过长。当这一点被解决之后，GPU 侧检索和评分才真正成为决定性能的主要阶段。
...
同时，多 GPU QPS 随 GPU 数量增加而提升，并不意味着系统获得了理想线性加速。原因在于 CPU 控制面仍需承担请求分发和最终小规模结果汇总，多 GPU 并发访问共享 PCIe/SSD 路径时会放大 I/O 竞争，GPU merge 或 CPU aggregation 也会分别带来跨卡通信和主机侧排序开销。因此，本文更稳妥地将相关结果解释为“具有吞吐扩展能力”，而不是把 replicated 或归并方案描述为理想线性扩展。
...
再次，replicated 架构的优势建立在索引仍可复制的条件上。一旦索引规模继续增大，或候选预算和中间张量占用进一步上升，系统就必须转向 sharded 路径，并接受更高的通信与归并复杂度。这意味着本文对多 GPU 主方案的判断带有明确的适用前提，而不是无条件成立。

此外，out-of-core 路径与拓扑感知 I/O 优化对部署条件依赖较强，其收益与 GPU--SSD--PCIe--NUMA 拓扑、驱动与 GDS 支持高度相关。同一套方法在不同硬件节点上的收益幅度可能存在明显波动，因此本章关于 I/O 路径的结论更强调方向正确，而不意味着具体数值在所有平台上都可等比例复现。
...
由此，本文结论更适合作为单机多 GPU 条件下的工程验证：replicated 主方案仍受显存与索引规模约束，out-of-core 和拓扑感知 I/O 的收益依赖具体硬件路径与 GDS 支持，超大规模真实金融负载验证仍有限。多机分布式场景中的跨机通信、分布式索引和集群调度尚未纳入本文范围，需要在后续工作中继续验证。

\section{本章小结}

本章的综合实验把全文前几章分散建立的判断收束到同一验证框架中。在本文设定的数据规模、硬件平台和候选预算条件下，FlashTOD 在保持合理检测效果的同时，表现出更适合大规模金融异常检测的系统组织方式；单卡优化的主要收益来自候选召回到异常评分之间的数据路径压缩；多 GPU 条件下，replicated 更适合作为吞吐优先场景的主方案，而 GPU merge 与拓扑感知 I/O 控制则主要用于容量受限或路径受限场景下维持扩展效果。
