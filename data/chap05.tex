...
在数据划分上，真实集和 1M 桥接实验采用固定随机种子下的训练/验证/测试集划分，以保证 PR-AUC、Recall@$k$ 与 Top-$k$ overlap 等效果指标可比。10M、50M、100M 的单卡和多 GPU 性能实验则使用固定索引集与固定查询集，不再沿用 train/val/test 口径，而是将查询持续注入系统以衡量 QPS、p99 延迟和资源利用率；这样可以将检测效果评估与系统压测明确区分。
...
为保证比较口径一致，所有实验首先保持数据源和标签定义一致；有效性实验共享相同训练、验证和测试划分，性能实验共享相同索引集与查询集。在不同方案对比中，候选预算、批大小、硬件环境和 CUDA、PyTorch、FAISS-GPU、RAPIDS、NCCL 等软件栈均保持一致；单卡与多 GPU 仅在相同规模下分别比较，每组实验使用统一重复轮次和统计方式汇总结果。
...
上述讨论限定了本文结论的适用边界。总体来看，replicated 主方案仍受显存容量和索引规模限制；out-of-core 与拓扑感知 I/O 优化虽然能够缓解路径争用，但收益依赖 GPU--SSD--PCIe--NUMA 等硬件路径和 GDS 支持条件；超大规模实验主要建立在高保真合成数据上，真实金融机构在线负载验证仍有限；此外，本文尚未覆盖多机分布式扩展，跨机通信、分布式索引管理和集群调度仍需后续研究进一步验证。
