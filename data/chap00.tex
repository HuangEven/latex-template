...
\begin{enumerate}
  ...
  \item 单 GPU 数据路径优化。本文针对“GPU 检索---CPU 重组---GPU 评分”的折返路径，设计 GPU 主导执行链路，使候选结果和评分中间张量尽量保持在GPU侧，并结合异步流水线、双缓冲、页锁定内存及 RAPIDS 数据后端降低必要边界开销，进一步优化数据路径。值得注意的是，FAISS-GPU 用于前期 device/host pointer 路径和候选接口验证，不作为最终外存召回主路径。
