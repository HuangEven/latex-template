...
金融异常检测面对的对象并不抽象。它可能是一笔金额和地域组合异常的交易，也可能是账户在短时间窗内突然改变的行为序列，或由设备指纹、登录地点和交易对手共同构成的高风险模式。在实际的检测系统中，检测效果与执行效率是同一业务约束的两个方面，而不是先后独立优化的目标\citep{aggarwal2017outlier,song2024financialriskstgnn}。考虑到吞吐不足会造成请求堆积，尾延迟过高则可能使告警错过处置窗口，高频交易流持续进入系统后，风险告警必须在有限时间内完成。
