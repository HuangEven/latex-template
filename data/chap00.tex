...
本文完成的工作按一条核心链路和两类性能实现机制组织。

\begin{enumerate}
  \item 本文提出并实现 FlashTOD 两阶段异常检测链路，这是全文的核心系统创新。FlashANNS 先从全量样本中返回规模受候选预算约束的候选集合，PyTOD 再完成候选集上的张量化精细评分，评分对象由全量规模 $n$ 收缩到 $|C_i|$。两类组件的输入输出语义并不相同：系统根据 candidate ID、distance、local rank 和 reference/index mapping 恢复原始样本与候选集合的对应关系，并构造 PyTOD 所需的候选张量、距离张量和评分中间张量。这一设备侧数据契约改变了高代价评分的执行范围，也规定了中间结果在两个阶段之间的表示与传递方式。
  \item 围绕 FlashTOD 链路实现单 GPU 数据通路优化。初始路径中的 GPU 检索、CPU 候选重组和 GPU 评分会形成 D2H/H2D 折返，并伴随格式转换、内存申请和阶段同步。本文让查询表示、候选缓冲和评分中间张量尽量驻留设备侧，结合异步 I/O---计算重叠、缓冲区复用和批次执行压缩热路径开销。FAISS-GPU 用于前期 device/host pointer 与候选接口验证，完整外存召回路径仍以 FlashANNS 为前端。
  \item 围绕 FlashTOD 链路实现单机多 GPU 并行扩展。索引可复制时，replicated 路径把查询分配到不同 GPU，每张设备独立完成候选召回与评分闭环；CPU 负责请求分发、负载均衡和小规模结果汇总。容量或部署条件限制完整复制时，系统再采用设备侧归并、NCCL 通信和 topology-aware I/O 控制。这里的请求分配属于调度，数据划分与本地计算属于并行执行，归并和拓扑约束则分别落在跨设备结果处理与数据通路。相关结论限定在本文的单机硬件和拓扑环境。
\end{enumerate}

上述工作改进的是大规模异常检测的系统执行方式，没有改变 PyTOD 所采用的异常评分模型。
